...
class label : public object<Context>
{
  ...
  public:
    ...
    draw() override
    {
        this->ui_context_.buffer().text(this->get_position().get_x(), this->get_position().get_y(), text_, {fg_, bg_});
    }

    void
    text(std::wstring const &text)
    {
        text_ = text;
    }

    void
    set_color(color const &fg, color const &bg)
    {
        fg_ = fg;
        bg_ = bg;
    }
