...
template <class BUFFER> class context {
public:
  using buffer_type = BUFFER;
  using frame_type = frame<context<BUFFER>>;
  using frame_container_type = std::vector<frame_type *>;
  using frame_iterator_container_type =
      std::vector<typename frame_type::frame_container_type::iterator>;
...
public:
    ...
    for (; !frames_stack_.empty();) {
      ...
      if (input.special(input::special_key::tab)) {
        ...
        if (input.special(input::special_key::shift))
          backward = true;
        typename frame_type::frame_container_type::iterator start_it =
            frames_stack_.back()->children().begin();
  ...
  template <class CONTEXT> void push(frame<CONTEXT> *frame) {
    frames_stack_.push_back(reinterpret_cast<frame_type *>(&frame));
    iterators_.push_back(frames_stack_.back()->first_focusable());
    for (typename frame_type::frame_container_type::value_type obj :
         frames_stack_.back()->children())
      obj->on_show();
