...
template <class CONTEXT> class frame : public object<CONTEXT> {
public:
...
public:
  frame(context_type &ui_context, attributes<context_type> const &attrs,
  ...
  virtual void draw() override {
    this->ui_context_.buffer().fill_area(this->get_attributes().get_x(),
  ...
  frame_container_type const &children() const { return children_; }

  frame_container_type &children() { return children_; }

  void add_child(object<CONTEXT> *ui_object) {
    children_.emplace_back(ui_object);
  }

  typename frame_container_type::iterator
  first_focusable(typename frame_container_type::iterator it,
                  bool backward = false) {
    if (children_.empty())
