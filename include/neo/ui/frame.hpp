...
class frame : public object<CONTEXT>
{
  ...
  public:
    ...
    draw() override
    {
        this->ui_context_.buffer().fill_area(this->get_position().get_x(), this->get_position().get_y(),
                                             this->get_position().get_width(), this->get_position().get_height());
        for (auto *child : children_)
            child->draw();
        // this->ui_context_.buffer().box(this->get_position().get_x(), this->get_position().get_y(),
        //                                this->get_position().get_width(), this->get_position().get_height());
    }

    frame_container_type const &
    children() const
    {
        return children_;
    }
    ...
    void
    add_child(object<CONTEXT> *ui_object)
    {
        children_.emplace_back(ui_object);
    }

    typename frame_container_type::iterator
    first_focusable(typename frame_container_type::iterator it, bool backward = false)
    {
        if (children_.empty())
