...
template <class CONTEXT> class button : public object<CONTEXT> {
public:
  using parent_type = object<CONTEXT>;
  using dimension_type =
      typename parent_type::context_type::buffer_type::size_type;
...
public:
  button(typename parent_type::context_type &ui_context,
         attributes<context_type> const &attrs,
         std::basic_string<
             typename CONTEXT::buffer_type::term_cell::char_type> const &text,
  ...
  virtual void draw() override {
    this->ui_context_.buffer().box(
        this->get_attributes().get_x() - 1, this->get_attributes().get_y() - 1,
        static_cast<typename CONTEXT::buffer_type::size_type>(text_.size() + 2),
        3);
    if (!focused_)
      this->ui_context_.buffer().text(this->get_attributes().get_x(),
                                      this->get_attributes().get_y(), text_,
