...
class button : public object<Context>
{
  ...
  public:
    ...
    draw() override
    {
        this->ui_context_.buffer().box(this->get_position().get_x() - 1, this->get_position().get_y() - 1,
                                       text_.size() + 2, 3);
        if (!focused_)
            this->ui_context_.buffer().text(this->get_position().get_x(), this->get_position().get_y(), text_,
