...
std::map<color, int> const colors = {
    {color::white, COLOR_WHITE}, {color::black, COLOR_BLACK},
    {color::red, COLOR_RED},     {color::green, COLOR_GREEN},
  ...
  WINDOW *init_win_hdl() {
    ...
    if (::start_color() != OK)
      throw std::runtime_error(
          "PDCurses init: could not initialize start_color");

    return win_hdl;
  }

public:
  WINDOW *win_hdl() const { return win_hdl_; }
...
public:
  HANDLE std_handle() { return std_handle_; }
...
public:
  gainput::InputManager &input_manager() { return input_manager_; }

  gainput::InputMap &input_map() { return input_map_; }

  p_impl() : input_map_(input_manager_) {
    win_hdl_ = init_win_hdl();

    const gainput::DeviceId keyboardId =
        input_manager_.CreateDevice<gainput::InputDeviceKeyboard>();

    input_map_.MapBool(input::special_key::backspace, keyboardId,
                       gainput::KeyBackSpace);
    input_map_.MapBool(input::special_key::enter, keyboardId,
                       gainput::KeyKpEnter);
    input_map_.MapBool(input::special_key::escape, keyboardId,
                       gainput::KeyEscape);
    input_map_.MapBool(input::special_key::shift, keyboardId,
                       gainput::KeyShiftL);
...
input context::read() {
  ...
  if (irec.EventType == KEY_EVENT &&
    ...
    input.specials()[input::special_key::shift] =
        (key.dwControlKeyState & SHIFT_PRESSED) == SHIFT_PRESSED;
...
input context::read() {
  ...
  input.specials()[input::special_key::shift] =
      p_impl_.get()->input_map().GetBoolWasDown(input::special_key::shift);
  input.specials()[input::special_key::tab] =
      p_impl_.get()->input_map().GetBoolWasDown(input::special_key::tab);
  input.specials()[input::special_key::escape] =
      p_impl_.get()->input_map().GetBoolWasDown(input::special_key::escape);
  input.specials()[input::special_key::enter] =
      p_impl_.get()->input_map().GetBoolWasDown(input::special_key::enter);
...
void context::print_ch16(unsigned int x, unsigned int y, char16_t ch,
                         std::pair<color, color> const &foreground_background) {
  ::init_pair(1, colors.at(foreground_background.first),
              colors.at(foreground_background.second));
  ::attron(COLOR_PAIR(1));
